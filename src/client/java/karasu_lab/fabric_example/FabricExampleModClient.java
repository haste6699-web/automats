...
public class FabricExampleModClient implements ClientModInitializer {
	...
	private int clicksPerSecond = 50;
	...
	private int getClickCooldownTicks() {
		return Math.max(0, Math.round(20.0F / clicksPerSecond) - 1);
	}

	private int getFastClickCooldownTicks() {
		return clicksPerSecond <= 20 ? getClickCooldownTicks() : 0;
	}

	private int getFastClickCountPerTick() {
		return Math.max(1, (int)Math.ceil(clicksPerSecond / 20.0D));
	}
	...
	private class AutoClickerConfigScreen extends Screen {
		...
		protected void init() {
			...
			addDrawableChild(ButtonWidget.builder(Text.literal("Done"), button -> close())
					.dimensions(buttonX, panelY + 98, buttonWidth, 20)
					.build());
		}
		...
		public void render(DrawContext context, int mouseX, int mouseY, float delta) {
			...
			context.drawCenteredTextWithShadow(textRenderer, Text.literal("Open GUI key and toggle key can be rebound in Controls."), width / 2, panelY + 27, 0xA0A0A0);
		...
		private Text getToggleText() {
			return Text.literal("Auto clicker: " + (enabled ? "ON" : "OFF"));
		}
	}

	private class SpeedSlider extends SliderWidget {
		private SpeedSlider(int x, int y, int width, int height) {
			super(x, y, width, height, Text.empty(), (clicksPerSecond - MIN_CLICKS_PER_SECOND) / (double)(MAX_CLICKS_PER_SECOND - MIN_CLICKS_PER_SECOND));
			updateMessage();
		}

		@Override
		protected void updateMessage() {
			setMessage(Text.literal("Speed: " + clicksPerSecond + " clicks/sec"));
		}

		@Override
		protected void applyValue() {
			clicksPerSecond = MIN_CLICKS_PER_SECOND + (int)Math.round(value * (MAX_CLICKS_PER_SECOND - MIN_CLICKS_PER_SECOND));
			clicksPerSecond = Math.clamp(clicksPerSecond, MIN_CLICKS_PER_SECOND, MAX_CLICKS_PER_SECOND);
		}
	}
}
